...
def search(request, job_id):
    try:
        ...
        return render(request, "ligand_service/search.html", {"status": "Job ID does not exist!", "job_id": job_id})

    if submission.finished_at is None:
        return render(request, "ligand_service/search.html", {"status": "Ongoing", "job_id": job_id})
    ...
    for form in submission.submittedform_set.all():
        ...
        with open(results_path.joinpath(f'result{file_id}.tsv'), newline='') as csvfile:
            ...
            reader = csv.DictReader(csvfile, skipinitialspace=True, 
                                    fieldnames=["frame", "interaction_type", "atom_1", "atom_2", "atom_3", "atom_4"], 
                                    delimiter="\t")
            raw_table = [{k: v for k, v in row.items()}
            for row in reader]
            if submission.common_numbering == True:
                ...
                for row in raw_table:
                    ...
                    for atom in ["atom_1", "atom_2"]:
                        ...
                        print(key)
                        if key in dic:
                            print(f"KEY: {key} IN THE DIC, VALUE: {dic[key]}", flush=True)
            ...
            data.append((file_id,form.value, table,))

    return render(request, "ligand_service/search.html", {"status": "Success", "job_id": job_id, "data": data, "name_VOI": submission.name_VOI})
